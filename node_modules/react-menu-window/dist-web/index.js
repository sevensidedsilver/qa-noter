import { useRef, useEffect, useState, useCallback, createElement, Fragment, cloneElement, Children } from 'react';
...
function useAnimationFrame(update) {
  ...
  var latestAnimationFrame = useRef(null);
  useEffect(() => {
...
var fitsOnBottom = viewportOffset => (elemRect, viewportRect) => elemRect.bottom < viewportRect.height - viewportOffset;
var fitsOnRight = viewportOffset => (elemRect, viewportRect) => elemRect.right < viewportRect.width - viewportOffset;
var getFitOffsets = viewportOffset => elemRect => {
  var viewportRect = {
...
function useFitInViewport(elemRef, targetPos) {
  ...
  var [style, setStyle] = useState(targetPos);
  useEffect(() => {
    setStyle(targetPos);
  }, [targetPos]);
  useAnimationFrame(() => {
...
var setEventListeners = (events, fn, elem, set) => {
...
function MenuWindow(_ref) {
  ...
  var latestEvent = useRef(null);
  var [pos, setPos] = useState(null);
  var contentRef = useRef(null);
  var openWindow = useCallback(e => {
    onOpen && onOpen(e);
    setPos(getPosition(e));
  }, [onOpen, setPos, getPosition]);
  var closeWindow = useCallback(e => {
    setPos(null);
    onClose && onClose(e);
  }, [setPos, onClose]);
  var handleContextMenu = useCallback(e => {
    e.preventDefault();
    e.stopPropagation();
    e.persist();
  ...
  return createElement(Fragment, null, cloneElement(Children.only(children), {
    onContextMenu: handleContextMenu
  }), renderedContent ? createElement(Portal, null, cloneElement(renderedContent, {
    ref: contentRef,
